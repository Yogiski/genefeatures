...
pub fn call_samtools_faidx(
    ...
    let args: [&str; 5] = ["faidx", "--fai-idx", &faidx, fasta, &region];
    let output: Output = Command::new("samtools").args(args).output()?;
    ...
    if output.status.success() {
        ...
        Err(format!(
            "Command `samtools {} {} {} {}` failed with error: {}",
            args[0], args[2], args[3], args[4], stderr
        )
        .into())
    }
}
