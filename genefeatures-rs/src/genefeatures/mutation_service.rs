...
lazy_static! {
    ...
    static ref INDEL: Regex = Regex::new(r"(\d+)_(\d+)delins([ACGT]+)|(\d+)_(\d+)del([ACGT]+)ins([ACGT]+)").unwrap();
}
...
pub fn process_mutation(mutation: String, seq: Seq<Dna>, seq_idx: SeqIdx) -> Seq<Dna> {
    ...
    let mut_regex: Option<&MutRegex> = MUT_REGEX
            .iter()
            .find(|mr| mr.is_match(&mutation));

    let (start, end, seq_ref, seq_alt): (usize, usize, Seq<Dna>, Seq<Dna>) =
...
fn mutate_sequence(seq: Seq<Dna>, start: usize, end: usize, seq_ref: Seq<Dna>, seq_alt: Seq<Dna>) -> Seq<Dna> {
    assert_eq!(
        &seq[start..end],
        &seq_ref,
...
fn dna_sub(_seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
    ...
    (start, end, seq_ref, seq_alt)
}

fn dna_point_del(seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
...
fn dna_range_del(seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
...
fn dna_ins(_seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
    let (start, end): (usize, usize) = (
...
fn dna_indel(seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
...
fn dna_dup(seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
    let (start, end): (usize, usize) = (
        seq_idx.mutation_index[recipe[0]] as usize,
        (seq_idx.mutation_index[recipe[1]] + 1) as usize
    );
...
fn dna_inv(seq: &Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> (usize, usize, Seq<Dna>, Seq<Dna>) {
    let (start, end): (usize, usize) = (
        seq_idx.mutation_index[recipe[0]] as usize,
        (seq_idx.mutation_index[recipe[1]] + 1) as usize
    );
