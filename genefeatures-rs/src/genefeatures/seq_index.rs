...
fn extend_mutation_index(mut_idx: &mut Vec<String>, start: u64, end: u64, prefix: String) {
    for i in start..end {
...
fn process_pre_tss_utr_and_first_cds(
    ...
    let (i_start, i_end): (u64, u64) = get_interval_indices(strand, rec.expect("No record found!"));
    // Process pre tss utr index
    extend_mutation_index(mutation_index, start.abs_diff(i_start), 0, "-".to_string());
    // process first cds
    let cds_len = i_end.abs_diff(i_start);
    extend_mutation_index(mutation_index, 1, cds_len, "".to_string());
    cds_idx += cds_len;
    (i_end, cds_idx)
}
...
fn process_intron_halves(
    ...
    extend_mutation_index(mutation_index, mid_point, 0, format!("{cds_idx}-"));
}
...
impl SeqIdx {
    ...
    fn init_mutation_index(
        ...
        process_cds_and_introns(&mut mutation_index, &mut cds, cds_idx, strand, last_cds_end);
