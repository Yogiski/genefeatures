...
mod tests {
    ...
    use genefeatures::gtf_tree::{GtfTree, Contig, Gene, Transcript, Cds, NonCds, Node};
    ...
    fn test_gtf_tree_from_file() {
        ...
        assert!(!gtf.contigs[0].genes[0].transcripts[0].cds_records.records.is_empty());
    }
    ...
    fn test_gtf_tree_find_transcript()
     {
        ...
        let mapping: HashMap<&str, GtfRecordValue> = vec![
            ...
            ("transcript_id", GtfRecordValue::OptStr(Some("ENST00000311936")))
        ].into_iter().collect(); 
        ...
        assert_eq!(search_res.unwrap().record.gene_name, Some("KRAS".to_string().into_boxed_str()));
