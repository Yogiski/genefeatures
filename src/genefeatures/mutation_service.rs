...
fn dna_sub(seq: Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> Seq<Dna> {
    let start: usize = seq_idx.sequence_index[recipe[0]] as usize;
...
fn dna_range_del(seq: Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> Seq<Dna> {
    let (start, end): (usize, usize) = (
        seq_idx.sequence_index[recipe[0]] as usize,
        (seq_idx.sequence_index[recipe[1]] + 1) as usize,
    );
...
fn dna_ins(seq: Seq<Dna>, seq_idx: SeqIdx, recipe: Vec<&str>) -> Seq<Dna> {
    let (start, end): (usize, usize) = (
        (seq_idx.sequence_index[recipe[0]] + 1) as usize,
        (seq_idx.sequence_index[recipe[1]]) as usize,
    );
