...
mod tests {
    ...
    fn test_forward_mut_geno_index_match() {
        ...
        assert_eq!(
            ...
            seq_idx.sequence_index.values().min()
        );
        assert_eq!(
            seq_idx.genomic_index.values().max(),
            seq_idx.sequence_index.values().max()
        );
    }
    ...
    fn test_reverse_seq_idx_matches_seq_len() {
        ...
        assert_eq!(seq_idx.sequence_index.len(), seq.len())
    }
    ...
    fn test_forward_seq_utr_tss_junction() {
        let (_, seq_idx): (Transcript, SeqIdx) = setup_seq_index("+");
        assert_eq!(
            seq_idx.sequence_index["1"],
            seq_idx.sequence_index["-1"] + 1
        );
    }
    ...
    fn test_forward_seq_first_pos() {
        let (_, seq_idx): (Transcript, SeqIdx) = setup_seq_index("+");
        assert_eq!(seq_idx.sequence_index["-261"], 0);
    }

    #[test]
    fn test_reverse_seq_first_pos() {
        let (_, seq_idx): (Transcript, SeqIdx) = setup_seq_index("-");
        assert_eq!(seq_idx.sequence_index["-5545"], 0);
    }
    ...
    fn test_reverse_seq_last_pos() {
        ...
        assert_eq!(seq_idx.sequence_index["*4552"], seq_len);
    }
    ...
    fn test_intron_indices_reverse() {
        let (_, seq_idx): (Transcript, SeqIdx) = setup_seq_index("-");
        assert_eq!(
            seq_idx.sequence_index["112-1"],
            seq_idx.sequence_index["112"] - 1
        );
        assert_eq!(
            seq_idx.sequence_index["111+1"],
            seq_idx.sequence_index["111"] + 1
        );
    }
    ...
    fn test_forward_seq_tss_idx_seq() {
        let (_, seq_idx): (Transcript, SeqIdx) = setup_seq_index("+");
        let seq: Seq<Dna> = read_forward_seq();
        let tss_i: usize = seq_idx.sequence_index["1"] as usize;
        let tss_f: usize = seq_idx.sequence_index["4"] as usize;
